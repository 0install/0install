...
let get_selections options ~refresh ?test_callback reqs mode =
  ...
  let select_with_refresh refresh =
    ...
    H.solve_and_download_impls (Lazy.force options.ui) driver ?test_callback reqs mode ~refresh in
...
let handle options flags arg ?test_callback for_op =
  ...
  let get_app_sels path =
    Zeroinstall.Apps.get_selections_may_update (Lazy.force options.driver) options.ui path in
