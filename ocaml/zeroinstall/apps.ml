...
let foreground_update ui driver app_path reqs =
  log_info "App '%s' needs to get new selections; current ones are not usable" app_path;
  match Helpers.solve_and_download_impls (Lazy.force ui) driver reqs `Download_only ~refresh:true |> Lwt_main.run with
...
*)
let check_for_updates ui driver app_path sels =
      ...
      let new_sels =
        ...
        | None ->
            ...
            if unavailable_sels then (
              ...
              foreground_update ui driver app_path reqs
            ) else (
...
let get_selections_internal system ?driver_ui app_path =
  ...
  if Sys.file_exists sels_path then
    ...
    match driver_ui with
    | None -> sels
    | Some (driver, ui) -> check_for_updates ui driver app_path sels
  else
    match driver_ui with
    | Some (driver, ui) -> foreground_update ui driver app_path (get_requirements system app_path)
...
let get_selections_may_update driver ui app_path =
  get_selections_internal driver#config.system ~driver_ui:(driver, ui) app_path
