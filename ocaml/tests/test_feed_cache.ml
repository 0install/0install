...
let suite = "feed-cache">::: [
  ...
  "check-signed">:: Fake_system.with_fake_config (fun (config, _fake_system) ->
    ...
    let fetcher = new Zeroinstall.Fetch.fetcher config trust_db distro download_pool Fake_system.null_ui in
    ...
    let dryrun_fetcher = new Zeroinstall.Fetch.fetcher {config with dry_run = true} trust_db distro download_pool Fake_system.null_ui in
