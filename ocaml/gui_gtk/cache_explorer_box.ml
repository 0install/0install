...
let open_cache_explorer config =
  ...
  let sorted_model = GTree.model_sort model in
  ...
  delete#connect#clicked ~callback:(fun () ->
    let iters = selection#get_selected_rows |> List.map (fun sorted_path ->
      sorted_model#get_iter sorted_path |> sorted_model#convert_iter_to_child_iter
    ) in
    dialog#misc#set_sensitive false;
    ...
    Gtk_utils.async ~parent:dialog (fun () ->
      try_lwt
        let rec loop = function
          | [] -> Lwt.return ()
          | x::xs ->
              let dir = model#get ~row:x ~column:impl_dir_col in
              lwt () = Lwt_preemptive.detach (U.rmtree ~even_if_locked:true config.system) dir in
              let removed = model#remove x in
              assert removed;
              loop xs in
  ...
  all_digests |> Hashtbl.iter (fun digest dir ->
    ...
    model#set ~row ~column:impl_dir_col @@ dir +/ digest;
    try
      let feed, impl = Hashtbl.find impl_of_digest digest in
      model#set ~row ~column:owner_col feed.F.name;
      model#set ~row ~column:version_str_col @@ F.get_attr_ex FeedAttr.version impl;
    with Not_found ->
      try
        Manifest.parse_digest digest |> ignore;
        model#set ~row ~column:owner_col "(unowned)";
      with _ ->
        model#set ~row ~column:owner_col "(temporary)";
  );
  ...
  lwt () =
    ...
    | Some row ->
        let rec loop () =
          let dir = model#get ~row ~column:impl_dir_col in
          lwt size = Lwt_preemptive.detach (size_of_impl config.system) dir in
          model#set ~row ~column:size_col size;
          model#set ~row ~column:size_str_col (U.format_size size);
          if model#iter_next row then loop ()
          else Lwt.return () in
        loop ()
    | None -> Lwt.return () in
